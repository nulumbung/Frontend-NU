...
export default function EditPostPage() {
    ...
    const fetchPostAndCategories = async () => {
      try {
        ...
        setFormData({
          ...
          is_spotlight: Boolean(data.is_spotlight),
          is_breaking: Boolean(data.is_breaking),
          is_headline: Boolean(data.is_headline)
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Failed to update post: ' + getErrorMessage(error));
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sm:p-8 space-y-6">
             ...
             <p className="text-xs text-gray-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Kredit Gambar Tambahan</label>
                    <input 
                      type="text" 
