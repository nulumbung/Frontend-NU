...
export default function EditBanomPage() {
  ...
  useEffect(() => {
    const fetchBanom = async () => {
      try {
        const response = await api.get(`/banoms/${params.id}`);
        ...
        console.error('Failed to fetch banom', error);
        alert('Failed to load banom data');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/banoms/${params.id}`, formData);
      router.push('/admin/banom');
    } catch (error: unknown) {
      console.error('Failed to update banom', error);
      alert('Failed to update banom: ' + getErrorMessage(error));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Edit Banom</h1>
      </div>
